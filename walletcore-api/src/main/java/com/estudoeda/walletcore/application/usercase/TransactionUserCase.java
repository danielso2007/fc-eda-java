...
public class TransactionUserCase {
    ...
    private final BalancesUserCase balancesUserCase;

    public TransactionUserCase(final TransactionGateway transactionGateway, final AccountGateway accountGateway,
            final BalancesUserCase balancesUserCase) {
        this.transactionGateway = transactionGateway;
        this.accountGateway = accountGateway;
        this.balancesUserCase = balancesUserCase;
    }
    ...
    public Transaction create(final UUID accountFromId, final UUID accountToId, BigDecimal amount) {
        ...
        var entity = transactionGateway.create(transaction);
        publicarEventoDeBalanco(accountFromId, accountToId, amount, entity.getId(), entity.getCreatedAt());
        return entity;
    }
    ...
    private void publicarEventoDeBalanco(final UUID accountFromId, final UUID accountToId, final BigDecimal amount,
            UUID transactionId, Instant createdAt) {
        balancesUserCase.sendBalanceDebit(accountFromId, amount, transactionId, createdAt);
        balancesUserCase.sendBalanceCredit(accountToId, amount, transactionId, createdAt);
    }
